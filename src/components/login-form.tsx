...
import {firebaseSignIn, firebaseSignOut} from '../modules/firebase/firebase-actions';
...
const LoginFormComponent: React.SFC<ILoginFormProps> =
  (props: ILoginFormProps): React.ReactElement<ILoginFormProps> => {
    const {email, password, inProgress, connected, user, dispatch} = props;

    const login = () => dispatch(firebaseSignIn(email, password));
    const signOut = () => dispatch(firebaseSignOut());
    const _onChange: React.ChangeEventHandler<HTMLInputElement> =
      (event) => dispatch(loginFormChange({[event.target.id]: event.target.value}));
    const style: React.CSSProperties = {
      display: 'flex',
      flexDirection: 'column'
    };

    if (!connected) {// firebse offline
      return (
        <div style={style}>
          <CircularProgress/>
        </div>
