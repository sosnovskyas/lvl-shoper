...
export const goodsEditWindowOpen = (
  payload: IGoodsListItem
): Action & { payload: IGoodsListItem } => ({
  type: GOODS_EDIT_WINDOW_OPEN,
  payload
});

export const goodsEditWindowSave = (
  payload: IGoodsListItem
): Action & { payload: IGoodsListItem } => ({
  type: GOODS_WINDOW_SAVE,
  payload
});
export const goodsWindowCancel = (): Action => ({
  type: GOODS_WINDOW_CANCEL
});
export const goodsWindowChange = (payload: {
  [key: string]: string;
